...
async function getVancouverOpenData(lat, lng) {
  ...
  try {
    ...
    const response = await fetch(
      `https://opendata.vancouver.ca/api/explore/v2.1/catalog/datasets/parking-meters/records?where=within_distance(geo_point_2d,%20geom%27POINT(${lng}%20${lat})%27,%20500m)&limit=20`,
    );
    ...
    // error message if there is a network error, no results, etc
